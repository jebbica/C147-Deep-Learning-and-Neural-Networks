  ...
  def loss(self, X, y=None):
    ...
    if self.use_batchnorm:
      for i in range(1, self.num_layers):
        ...
        X_temp, cache = affine_batch_relu_forward(X_temp, W, b, self.params[f'gamma{i}'], self.params[f'gamma{i}'], self.bn_params[i-1])  
        affine_batch_relu_caches.append(cache)
    else:
      for i in range(1, self.num_layers):
        W, b = self.params[f'W{i}'], self.params[f'b{i}']
    ...
    if self.use_batchnorm:
      for i in range(self.num_layers - 1, 0, -1):
          ...
          dx, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = affine_batch_relu_backward(dx, affine_batch_relu_caches[i - 1])

...
def adam(w, dw, config=None):
  ...
  v_biased = (1/(1-config['beta1']**config['t'])) * config['v']

  config['a'] = config['beta2'] * config['a'] + (1-config['beta2']) * dw * dw
  a_biased = (1/(1-config['beta2']**config['t'])) * config['a']

  next_w = w - (config['learning_rate'] * v_biased / (np.sqrt(a_biased + config['epsilon'])))

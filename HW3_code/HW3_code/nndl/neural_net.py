...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h1 = np.dot(W1, X.T) + b1[:, np.newaxis]
    h1_relu =  np.maximum(0, h1)
    h2 = np.dot(W2, h1_relu) + b2[:, np.newaxis]
    scores = h2.T
    ...
    indic = (h1 > 0).astype(int)
    dh1 = np.dot(dscores,W2)*indic.T


    grads['W2'] = np.dot(h1_relu, dscores).T
    grads['b2'] = np.sum(dscores, axis = 0)

    
    grads['W1'] = np.dot(X.T, dh1).T

...
def conv_forward_naive(x, w, b, conv_param):
  ...
  - conv_param: A dictionary with the following keys:
    ...
    - 'pad': The number of pixels tat will be used to zero-pad the input.
      ...
      for f in range(num_filts):
             ...
             for h_out in range(H_out):
                ...
                for w_out in range(W_out):
                  ...
                  dw[f] += xpad[n,:,h_start:(h_start+f_height), w_start:(w_start+f_width)] * upstream_region
                  dxpad[n,:,h_start:(h_start+f_height), w_start:(w_start+f_width)] += w[f] * upstream_region
     ...
     for f in range(C):
        for h_out in range(H_out):
           ...
           for w_out in range(W_out):
              ...
              region = x[n, f, hstart:hstart+pool_height, wstart:wstart+pool_width]
              m = np.max(region)
              dx[n, f, hstart:hstart+pool_height, wstart:wstart+pool_width] += (m == region) * upstream
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  #   implemented in HW #4.
  # ================================================================ #
  mode, eps, momentum = bn_param['mode'], bn_param.get('eps', 1e-5), bn_param.get('momentum', 0.9)
  N, C, H, W = x.shape

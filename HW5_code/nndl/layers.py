...
def affine_forward(x, w, b):
  ...
  N = x.shape[0]
  x_reshaped = x.reshape(N, -1) #reshape X to be (N, d_1 * ... * d_k) in this case, 2,120
...
def batchnorm_backward(dout, cache):
  ...
  da = 1/(np.sqrt(var + eps)) * dx_norm

  dmean = -1/(np.sqrt(var+eps)) * np.sum(dx_norm, axis=0)
  dvar = np.sum((-0.5/((var+eps)**1.5))*(x-mean)*dx_norm, axis=0)

  dx = da + (2*(x-mean)/N) * dvar + (dmean/N)
